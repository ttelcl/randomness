...
let usage detail =
  ...
  if showSynopsis "extract" then
    cp "\fowikidata extract\f0 \fg-wiki\f0 <\fcwiki\fy-\fcdate\f0> [\fg-raw\f0] [\fg-wrap\f0] {\fg-p\f0 <\fcposition\f0>|\fg-i\f0 <\fcindex\f0>}"
  if showDescription "extract" then
    ...
    cp "   \fg-wrap\f0\fx\fx                     Prepend the header and append the trailer (to create valid XML)"
    ...
    cp "                                         \fc0\f0 is the header and \fc-1\f0 the trailer"
