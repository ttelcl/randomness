...
public static class ByteSourceExtensions
{
  ...
  /// <summary>
  /// Return a sequence of values generated by cycling through the lists repeatedly
  /// and picking a random value from each list, until the total entropy used from
  /// <paramref name="randomSource"/> exceeds <paramref name="minimumEntropy"/> bits.
  /// </summary>
  /// <typeparam name="T">
  /// The item type stored in the lists
  /// </typeparam>
  /// <param name="randomSource">
  /// The source of randomness
  /// </param>
  /// <param name="minimumEntropy">
  /// The minimum entropy in bits to consume from the randomness source before
  /// terminating the sequence.
  /// </param>
  /// <param name="lists">
  /// The lists to cyclicly pick a random value from. There must be at least one
  /// list, each list must contain at least two items, and all items should be
  /// distinct.
  /// </param>
  /// <returns>
  /// The sequence of randomly selected items
  /// </returns>
  public static IEnumerable<T> RandomPickSequence<T>(
    this BitSource randomSource, double minimumEntropy, IEnumerable<IReadOnlyList<T>> lists)
  {
    var sources = lists.ToList();
    if(!sources.Any())
    {
      throw new ArgumentOutOfRangeException(nameof(lists), "Expecting at least one list");
    }
    if(sources.Any(source => source.Count < 2))
    {
      throw new ArgumentOutOfRangeException(nameof(lists), "All lists must have at least two elements");
    }
    var entropies = sources.Select(list => Math.Log2(list.Count)).ToList();
    var idx = 0;
    var entropy = 0.0;
    while(entropy < minimumEntropy)
    {
      var source = sources[idx];
      entropy += entropies[idx];
      idx = (++idx) % sources.Count;
      var selectedIndex = randomSource.RandomInteger(source.Count-1);
      yield return source[selectedIndex];
    }
  }
